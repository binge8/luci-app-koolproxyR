-- Copyright 2018 binge (ooxxbbbbb@gmail.com)
...
if luci.sys.call("pidof koolproxy >/dev/null") == 0 then
	status = translate("<strong class=\"koolproxy_status\"><font color=\"green\">广告过滤VIP 运行中</font></strong>")
else
	status = translate("<strong class=\"koolproxy_status\"><font color=\"red\">广告过滤VIP 未运行</font></strong>")
end

o = Map("koolproxy", "<font color='green'>" .. translate("广告过滤VIP ") .."</font>",     "<font color='purple'>" .. translate( "广告过滤VIP是能识别Adblock规则的广告屏蔽软件，可以过滤网页广告、视频广告、HTTPS广告") .."</font>")
